...
# Twitter项目推文数据爬取服务启动脚本
# 使用方法: ./start_service_project.sh [start|stop|restart|status] [interval]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

SERVICE_NAME="twitter-crawler-project"
PID_FILE="$SCRIPT_DIR/${SERVICE_NAME}.pid"
LOG_FILE="$SCRIPT_DIR/service_project.log"
...
start_service() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        caffeinate -i nohup "$SCRIPT_DIR/venv/bin/python" main.py --mode project-schedule \
            --interval $interval \
            --max-pages $max_pages \
            --page-size $page_size \
        ...
        # Linux系统
        nohup nice -n -5 "$SCRIPT_DIR/venv/bin/python" main.py --mode project-schedule \
            --interval $interval \
            --max-pages $max_pages \
            --page-size $page_size \
            --hours-limit $hours_limit > "$LOG_FILE" 2>&1 &
    ...
    sleep 3
    if ps -p $pid > /dev/null 2>&1; then
        print_success "项目推文服务启动成功 (PID: $pid)"
...
setup_monitoring() {
    ...
    # 添加新的监控任务
    echo "# Twitter Project Crawler Service Monitor" >> "$temp_cron"
...
run_once() {
    ...
    "$SCRIPT_DIR/venv/bin/python" main.py --mode project-once \
        --max-pages $max_pages \
        --page-size $page_size \
...
show_help() {
    ...
    echo "    - 跳过topic_id和project_id计算逻辑"
    echo "    - 跳过narrative_id、is_announce、is_real_project_tweet计算"
    echo "  • 保留智能时间检测和UTC时间转换功能"
    echo ""
    echo "智能时间检测功能:"
