...
# 使用方法: ./start_service_project_twitterapi.sh [start|stop|restart|status|once|logs|monitor]
...
show_help() {
    ...
    echo ""
    echo "命令:"
    echo "  start [间隔] [页数] [每页条数] [小时限制]  启动服务 (默认: 15分钟, 2页, 20条, 0.25小时)"
    echo "  stop                                   停止服务"
    echo "  restart [间隔] [页数] [每页条数] [小时限制] 重启服务"
    echo "  status                                 查看服务状态"
    echo "  once [页数] [每页条数] [小时限制]         执行单次爬取 (默认: 2页, 20条, 0.25小时)"
