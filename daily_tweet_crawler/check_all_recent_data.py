...
WHERE update_time >= '2025-12-25 18:00:00'
GROUP BY hour_group
ORDER BY hour_group
"""
...
logger.info("=" * 80)
logger.info("昨天晚上到今天 twitter_tweet_back_test_cmc300 表入库数据统计（按小时分组）")
logger.info("=" * 80)
...
# 同时检查需要修复的数据（user_id 有值但 user_name 为空的）
sql2 = """
...
if results2:
    need_fix = results2[0]['need_fix_count']
    logger.info("")
    logger.info(f"⚠️  需要修复 user_name 的数据: {need_fix} 条 (user_id有值但user_name为空)")
