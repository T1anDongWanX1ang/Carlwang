...
"""
获取 KOL 关注列表并入库
从 twitter_list_members_seed 表读取所有 KOL，查询其关注列表，存入 twitter_kol_all 表
...
使用方法：
    ...
    python fetch_kol_followings_new.py --dry-run --limit 1

    # 正式运行（处理所有KOL，自动使用缓存）
    python fetch_kol_followings_new.py

    # 正式运行（指定处理数量）
    python fetch_kol_followings_new.py --limit 10

    # 从缓存恢复（只入库已缓存的数据，不调用API）
    python fetch_kol_followings_new.py --resume

    # 清理所有缓存和进度
    python fetch_kol_followings_new.py --clear-cache

    # 查看缓存状态
    python fetch_kol_followings_new.py --cache-status
"""
import sys
...
# 添加项目根目录到Python路径
# daily_kol_following_crawler 在 Carlwang 目录下，需要向上一级找到 src 模块
...
class KOLFollowingsFetcher:
    ...
    def __init__(self, api_key: str, cache_dir: str = ".kol_cache_new"):
        """
        初始化获取器
        ...
        self.success_kols = 0
        self.failed_kols = 0
        self.skipped_kols = 0  # 跳过的KOL数（following > 2000）
    ...
    def fetch_all_kol_followings(self,
        ...
        try:
            ...
            self.logger.info(f"从 twitter_list_members_seed 表获取到 {self.total_kols} 个KOL")
    ...
    def _get_all_kols(self) -> List[Dict[str, Any]]:
        ...
        try:
            sql = """
            SELECT twitter_user_id as id, username as user_name, name, followers_count as followers, following_count as `following`
    ...
    def _get_followings_with_cache(self, user_name: str, use_api: bool = True, max_followings: int = None, kol_following_count: int = 0) -> List[Dict[str, Any]]:
        ...
        if use_api:
            ...
            # 注意：API 调用次数已在 _fetch_followings 方法内部统计
    ...
    def _fetch_followings(self, user_name: str, page_size: int = 200, max_followings: int = None, kol_following_count: int = 0) -> List[Dict[str, Any]]:
        """
        调用第三方API获取关注列表（支持分页获取数据，应用分层策略）
        ...
        cursor = ""
        page_num = 1
        api_calls_count = 0  # 记录本次实际API调用次数
        ...
        try:
            while True:
                # 检查是否已达到上限
                if final_max and len(all_followings) >= final_max:
                    self.logger.info(f"  已达到分层上限 {final_max}，停止获取")
                    break
                ...
                self.logger.debug(f"  API请求(第{page_num}页): {url}?userName={user_name}&pageSize={page_size}&cursor={cursor[:20] if cursor else ''}...")

                response = requests.get(url, params=params, headers=headers, timeout=30)
                api_calls_count += 1  # 每次实际调用 API，计数器 +1
            ...
            # 截断到最大限制
            if final_max and len(all_followings) > final_max:
                all_followings = all_followings[:final_max]
                self.logger.info(f"  已截断到 {final_max} 条（分层策略）")

            # 记录实际API调用次数
            self.api_calls += api_calls_count
            self.logger.info(f"  本次共调用 API {api_calls_count} 次，获取 {len(all_followings)} 条 following")

            return all_followings

        except requests.RequestException as e:
            self.logger.error(f"  API请求异常: {e}")
            self.api_calls += api_calls_count  # 即使失败也要记录已调用的次数
    ...
    def _show_statistics(self, test_mode: bool = False, dry_run: bool = False, resume_mode: bool = False):
        ...
        self.logger.info(f"跳过: {self.skipped_kols} (following > 2000)")
