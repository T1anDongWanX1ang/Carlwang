...
SERVICE_SCRIPT="$SCRIPT_DIR/start_service_kol_tweet.sh"
...
if check_service; then
    log_monitor "✓ KOL推文爬取服务运行正常"
else
    log_monitor "✗ KOL推文爬取服务已停止，尝试重启..."
    ...
    if [ -f "$SERVICE_SCRIPT" ]; then
        ...
        if check_service; then
            log_monitor "✓ KOL推文爬取服务重启成功"
        else
            log_monitor "✗ KOL推文爬取服务重启失败"
        fi
    else
