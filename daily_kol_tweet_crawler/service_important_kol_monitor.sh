...
SERVICE_SCRIPT="$SCRIPT_DIR/start_service_important_kol.sh"
PID_FILE="$SCRIPT_DIR/twitter-crawler-important-kol.pid"
MONITOR_LOG="$SCRIPT_DIR/monitor_important_kol.log"

PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
EXPECTED_MODE="--list-id 2009544306269548633"
...
if check_service; then
    log_monitor "✓ 重点KOL推文爬取服务运行正常"
else
    log_monitor "✗ 重点KOL推文爬取服务已停止，尝试重启..."
    ...
    if [ -f "$SERVICE_SCRIPT" ]; then
        ...
        if check_service; then
            log_monitor "✓ 重点KOL推文爬取服务重启成功"
        else
            log_monitor "✗ 重点KOL推文爬取服务重启失败"
        fi
    else
