...
start_timer_service() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        caffeinate -i nohup "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py schedule $interval > "$LOG_FILE" 2>&1 &
    else
        # Linux系统
        nohup nice -n -5 "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py schedule $interval > "$LOG_FILE" 2>&1 &
    fi
...
start_daemon_service() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS系统
        print_info "检测到macOS，使用防休眠启动..."
        caffeinate -i nohup "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py daemon > "$LOG_FILE" 2>&1 &
    else
        # Linux系统
        nohup nice -n -5 "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py daemon > "$LOG_FILE" 2>&1 &
    fi
...
restart_service() {
    ...
    # 默认使用定时器模式启动
    if [[ "$OSTYPE" == "darwin"* ]]; then
        caffeinate -i nohup "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py timer > "$LOG_FILE" 2>&1 &
    else
        nohup nice -n -5 "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py timer > "$LOG_FILE" 2>&1 &
    fi
...
run_once() {
    ...
    "$SCRIPT_DIR/venv/bin/python" scripts/run_marco.py now 2>&1 | tee -a "$LOG_FILE"
